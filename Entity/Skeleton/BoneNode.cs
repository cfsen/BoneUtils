...
public class BoneNode {
	...
	public void Rotate(Quaternion rotation, Func<Vector3, Quaternion, Vector3>? positionTransformer = null) {
		if (positionTransformer != null)
			Transform.BatchRotatePropagation(positionTransformer(Transform.Position, rotation), rotation);
		else {
		...
		foreach (var child in Children.Values)
			child.Rotate(rotation, positionTransformer);
	}
}
